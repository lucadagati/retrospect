...
void handle_start_command(const char *payload) {
  ...
  if (strlen(t.file) > 0) {
    ...
    execute_wasm_module(t.id, t.name, wasm_binary, wasm_decoded_len, t.inputs,
                        t.inputs_count);
...
int handle_registry_response(const char *payload) {
  ...
  execute_wasm_module(g_current_task.id, g_current_task.name, binary_data,
                      actual_decoded_len, g_current_task.inputs,
...
void mqtt_client_process(void) {
  if (mqtt_connected) {
    /* Cap poll timeout so the main loop can send alive/metrics on schedule. */
