...
pub struct HostFunctionRegistry {
    /// PX4/MAVLink functions
    pub px4_functions: HashMap<String, Px4Function>,
    /// microROS/DDS functions
    pub microros_functions: HashMap<String, MicroRosFunction>,
...
impl std::fmt::Debug for HostFunctionRegistry {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HostFunctionRegistry")
            .field("px4_functions", &self.px4_functions.len())
            .field("microros_functions", &self.microros_functions.len())
...
pub type Px4Function = Box<dyn Fn(&[u8]) -> WasmResult<Vec<u8>> + Send + Sync>;
pub type MicroRosFunction = Box<dyn Fn(&[u8]) -> WasmResult<Vec<u8>> + Send + Sync>;
...
impl HostFunctionRegistry {
    ...
    pub fn new(_config: &HostFunctionConfig) -> Self {
        Self {
            px4_functions: HashMap::new(),
            microros_functions: HashMap::new(),
