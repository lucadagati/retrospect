...
impl<'a> InterfaceName<'a> {
    ...
    /// Note: Version parsing is disabled in no_std build
    pub fn version(&self) -> Option<String> {
        let at = self.0.find('@')?;
        Some(self.0[at + 1..].to_string())
    }
}
...
impl<'a> ComponentNameParser<'a> {
    ...
    fn semver(&self, s: &str) -> Result<String> {
        // Simple semver validation without external crate (no_std compatible)
        // Basic format: MAJOR.MINOR.PATCH[-PRERELEASE][+BUILD]
        if s.is_empty() {
            bail!(self.offset, "`{s}` is not a valid semver: empty string");
        }
        // Basic validation: at least one dot, numbers and dots/hyphens/plus
        let has_dot = s.contains('.');
        let is_valid_chars = s.chars().all(|c| c.is_ascii_digit() || c == '.' || c == '-' || c == '+');
        if !has_dot || !is_valid_chars {
            bail!(self.offset, "`{s}` is not a valid semver: invalid format");
        }
        Ok(s.to_string())
    }
