...
check_pod_health() {
    ...
    if [ "$restarts" -eq 0 ]; then
        ...
        echo " Pod $pod_name has $restarts restarts"
    fi
...
then
    ...
    echo " Could not test controller health endpoints"
fi

# Check resource usage
echo " Checking resource usage..."
kubectl top pods -n wasmbed 2>/dev/null || echo " Resource metrics not available"
