...
\begin{itemize}
    ...
    \item the measurement harness output \texttt{experiments/scalability\_metrics\_1778938959.json}, which stores all 50 raw trial records and aggregate statistics used in the manuscript;
...
To regenerate the four figures included in this manuscript, a reviewer can execute:
\begin{verbatim}
cd paper_ieee_inginf05
../.venv-paper/bin/python generate_figures.py \
  --input ../experiments/scalability_metrics_1778938959.json
latexmk -pdf main.tex
\end{verbatim}
The script writes \texttt{latency\_ci\_profile.png}, \texttt{latency\_boxplot.png}, \texttt{latency\_cdf.png}, and \texttt{latency\_trial\_scatter.png} under \texttt{paper\_ieee\_inginf05/figures/}. All four are directly consumed by the LaTeX source. The CDF figure shows the empirical distribution including p95/p99 reference lines; the per-trial scatter demonstrates absence of temporal drift, confirming the stationarity assumption required for the Student-$t$ confidence intervals.
